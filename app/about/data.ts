...
export const projects: Project[] = [
  {
    ...
    highlights: [
      'DAU 2,200명의 사용자를 보유한 커뮤니티 서비스의 차세대 기술 전환(WordPress → React)과 UI/UX 리뉴얼에 참여',
      '마이페이지 (회원 정보 수정, 보유 포인트 조회, 작성 글/댓글 조회, 회원 탈퇴 등) 구현',
      '테스트맵을 기반으로 다양한 기기와 사용 환경에서 알파·베타 테스트를 진행하며 서비스 안정화에 기여',
      '리뉴얼 만족도 조사 결과, 리뉴얼 후 만족도 2.98 → 4.80 상승',
    ],
    links: {
  ...
  },
  {
    ...
    highlights: [
      '참석자 출석체크 페이지 구현 (학번 입력, 서명, 출석 확인 완료 모달)',
      '관리자 대시보드 페이지 구현',
      'chart.js를 사용한 행사별 통계 페이지 구현',
      'React 라이브러리 react-signature-canvas를 사용하여 전자 서명 터치 패드 기능 구현',
      '행사 목록 페이지 및 검색 기능 구현',
    ],
    links: {
  ...
  },
  {
    ...
    highlights: ['Notion API 연동을 통한 블로그 구현', '무한 스크롤 구현'],
    links: {
